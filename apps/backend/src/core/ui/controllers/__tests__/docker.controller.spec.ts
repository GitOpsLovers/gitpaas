...
describe('DockerController', () => {
    let mockDockerService: jest.Mocked<Pick<DockerService, 'info'>>;
    ...
    describe('getStatus', () => {
        it('delegates to the service to fetch the daemon info', async () => {
            mockDockerService.info.mockResolvedValue(dockerInfo);

            await sut.getStatus();

            expect(mockDockerService.info).toHaveBeenCalledTimes(1);
        });
        ...
        it('reflects zeroed counts and empty strings from the daemon info', async () => {
            mockDockerService.info.mockResolvedValue({
                ServerVersion: '',
                OperatingSystem: '',
                Containers: 0,
                Images: 0,
        ...
        it('rethrows a ServiceUnavailableException raised by the service unchanged', async () => {
            ...
            mockDockerService.info.mockRejectedValue(original);
        ...
        it('wraps non-Error rejection values into a ServiceUnavailableException', async () => {
            mockDockerService.info.mockRejectedValue('boom');
