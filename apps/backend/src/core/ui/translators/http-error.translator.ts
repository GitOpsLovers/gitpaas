...
const DOMAIN_ERROR_TRANSLATIONS = new Map<string, DomainErrorTranslation>([
    ...
    ['DEPLOYMENT_NOT_FOUND', (error) => new NotFoundException(error.message, { cause: error })],
    ['SERVICE_NAME_TAKEN', (error) => new ConflictException(error.message, { cause: error })],
    ...
    ['PROFILE_NOT_FOUND', (error) => new NotFoundException(error.message, { cause: error })],
    ['EMAIL_TAKEN', (error) => new ConflictException(error.message, { cause: error })],
    ['INVALID_CURRENT_PASSWORD', (error) => new UnauthorizedException(error.message, { cause: error })],
    ['INVALID_TWO_FACTOR_CHALLENGE', (error) => new UnauthorizedException(error.message, { cause: error })],
    ['INVALID_TOTP_CODE', (error) => new UnauthorizedException(error.message, { cause: error })],
    ['TOTP_NOT_STARTED', (error) => new ConflictException(error.message, { cause: error })],
    ['TOTP_ALREADY_ENABLED', (error) => new ConflictException(error.message, { cause: error })],
    ['USER_NOT_FOUND', (error) => new NotFoundException(error.message, { cause: error })],
