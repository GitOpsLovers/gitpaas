...
/**
 * Replay/live merger of a deployment's log stream.
 *
...
export class LogReplayMerger {
     ...
     */
    public merge(streamId: string, state: StreamState, subscriber: Subscriber<LogEvent>): () => void {
        let replaying = true;
        let ended = false;
        let liveEnded = false;
        let maxSeq = 0;
