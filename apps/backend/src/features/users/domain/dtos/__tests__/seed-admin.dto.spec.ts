...
import { SeedAdminDto } from '../seed-admin.dto';
...
const validatePayload = (payload: Record<string, unknown>): ValidationError[] =>
    validateSync(plainToInstance(SeedAdminDto, payload), {
        whitelist: true,
...
describe('SeedAdminDto', () => {
    ...
    it('rejects a malformed email', () => {
        const errors = validatePayload(validPayload({ email: 'admin(at)gitpaas.dev' }));
    ...
    it('rejects a non-string password', () => {
        const errors = validatePayload(validPayload({ password: 123456 }));

        expect(constraintsFor(errors, 'password')).toContain('isString');
    });

    it('accepts a short password, as no length rule is declared', () => {
        expect(validatePayload(validPayload({ password: 'a' }))).toEqual([]);
    });
