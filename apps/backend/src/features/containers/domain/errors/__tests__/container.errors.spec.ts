import { ServiceNotFoundError } from '../container.errors';

import { ServiceNotFoundError as DeploymentServiceNotFoundError } from '@features/deployments/domain/errors/deployment.errors';
...
describe('ServiceNotFoundError', () => {
    ...
    it('is never an instance of the deployments error of the same name', () => {
        expect(new ServiceNotFoundError(serviceId)).not.toBeInstanceOf(DeploymentServiceNotFoundError);
    });
});
