...
const daemonName = `api_${daemonKey}`;
...
describe('getVolumesByServiceUseCase', () => {
    ...
    it('builds the name of the daemon from the slug of the service and the key of the volume', async () => {
    ...
    it('gives the state orphan to a volume of the daemon that the database does not hold', async () => {
        mockDaemonVolumesRepository.listByService.mockResolvedValue([daemonVolume({ name: 'api_pgdata' })]);
    ...
    it('names the containers that mount an orphan', async () => {
        mockDaemonVolumesRepository.listByService.mockResolvedValue([daemonVolume({ name: 'api_pgdata' })]);
        mockDaemonVolumesRepository.listMountsByService.mockResolvedValue([
            { volumeName: 'api_pgdata', containerName: 'api-db-1' },
        ]);
