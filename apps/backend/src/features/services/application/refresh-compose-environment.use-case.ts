...
/**
 * Use case that reads the compose file of the repository of a service and caches the names and the values of its key `environment`.
 *
 ...
 */
export async function refreshComposeEnvironmentUseCase(
    servicesRepository: ServicesRepository,
    providersRepository: ProvidersRepository,
    providerClient: ProviderClient,
    repositoryComposeFile: RepositoryComposeFile,
    serviceId: string,
): Promise<ComposeEnvironmentCache | null> {
    ...
    const cache: ComposeEnvironmentCache = {
        variables: parseComposeEnvironmentUseCase(text),
        refreshedAt: new Date(),
    };

    await servicesRepository.saveComposeEnvironment(serviceId, cache);
