...
import { refreshComposeDomainsUseCase } from '../refresh-compose-domains.use-case';
...
describe('refreshComposeDomainsUseCase', () => {
    ...
    const run = (): Promise<ComposeDomainsCache | null> => {
    ...
    it('caches the domains and the moment of the read', async () => {
        await run();

        expect(mockServicesRepository.saveComposeDomains).toHaveBeenCalledTimes(1);
        expect(mockServicesRepository.saveComposeDomains).toHaveBeenCalledWith(serviceId, {
    ...
    it('returns the cache it wrote', async () => {
        await expect(run()).resolves.toEqual({ domains: declarations, refreshedAt });
    });

    it('caches an empty list when the compose file declares no domain', async () => {
        mockRepositoryComposeFile.read.mockResolvedValue('services:\n  web:\n    image: nginx\n');

        await expect(run()).resolves.toEqual({ domains: [], refreshedAt });
        expect(mockServicesRepository.saveComposeDomains).toHaveBeenCalledWith(serviceId, { domains: [], refreshedAt });
    });
