...
describe('readErrorPayloadUseCase', () => {
    it('reads the status and the parsed body of an HttpErrorResponse', () => {
    ...
    it('reports no status when the call itself failed and no answer arrived', () => {
        expect(readErrorPayloadUseCase({ status: 0, error: null })).toEqual({ status: null, body: null });
    });

    it('reports no body when the answer carries none', () => {
        expect(readErrorPayloadUseCase({ status: 503 })).toEqual({ status: 503, body: null });
    });

    it('degrades into no answer when the error is not an object', () => {
