...
The identifier is a UUID that the database generates. The three fields of the deployment start as an empty
text, because a caller gives them after the creation.
...
The body holds the name and the identifier of the project. The system SHALL set the identifier of the
repository, the deployment branch and the path of the compose file to an empty text. Thus a new service is
not deployable, and a caller makes it deployable with a later change.

#### Scenario: The body is correct

- **WHEN** a client posts a name and the identifier of an available project
...
- **WHEN** a client posts a body without a name, with an empty name, without a `projectId`, or with a
  `projectId` that is no UUID
- **THEN** the system answers `400 Bad Request`
...
The body holds the name, which is obligatory. The body can also hold the identifier of the repository, the
deployment branch and the path of the compose file. The system SHALL change only the fields that the body
holds.

A caller makes a service deployable with this operation, because it gives the identifier of the repository
...
- **THEN** the system changes the name, and it keeps the identifier of the repository, the deployment
  branch and the path of the compose file
