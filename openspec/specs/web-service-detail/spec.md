...
The tab `provider` SHALL give a form with three controls:

| Control | Kind |
|---|---|
| The repository | A list of the repositories that the installation can reach |
...
The system SHALL send the name of the service together with the three values, because the API asks for the
...
- **WHEN** the API accepts the three values
