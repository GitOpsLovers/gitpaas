...
Each user carries the role `admin` or the role `user`. The system SHALL NOT restrict any action by the
role at this time. Each authenticated user can do each action.

The token carries the role, so a later change can add the restrictions.

#### Scenario: A user with the role `user` calls any endpoint

- **WHEN** an active user with the role `user` calls a protected endpoint
