...
**`COMPOSE_PROJECT_LABEL` / slug helper are shared exports — import them, never re-declare.** The Compose label keys (`COMPOSE_PROJECT_LABEL`, `COMPOSE_SERVICE_LABEL`) are exported module-level members of `@core/infrastructure/docker/docker.client`, and the slug helper is `composeProjectNameUseCase` in `@core/application/compose-project-name.use-case`. Specs **import the real symbols** rather than declare local copies, so a renamed key or a changed slug rule fails the spec instead of silently drifting from the SUT (see `orphan-containers-docker.adapter.spec.ts`).
