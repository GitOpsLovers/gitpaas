...
- **Create:** `create` called with the input (including any DTO mutation the SUT performs — e.g. deployments injecting `status: 'pending'`), `save` called with the created entity, mapped result asserted. **Bulk create** (`createMany`): the DTO array is passed straight through `create` and `save`.
  - **Overloaded `create` typing:** TypeORM's `Repository.create` is overloaded (`create(entityLike): Entity` and `create(entityLikeArray): Entity[]`). Under `jest.Mocked<Pick<Repository<Entity>, 'create' | …>>` the overload set collapses to the single-entity signature, so a single-entity `create` stub types cleanly, but the array-returning bulk stub must localize a cast: `(mockRepository.create as jest.Mock).mockReturnValue(entities)`. Keep the single-entity stub fully typed and do NOT broaden the mock's declared type — the cast stays localized to the one bulk stub. Reference spec: `logs-db.repository.spec.ts`.
