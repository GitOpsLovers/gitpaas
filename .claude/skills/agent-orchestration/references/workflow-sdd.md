...
The command `.claude/commands/implement.md` holds this step. It takes the first phase of `TODO.md` that holds an open box, groups the tasks with [grouping.md](grouping.md), and delegates. `implementer` writes the code and its tests in the same call. Then the command invokes `git-manager`, which creates the branch, the commit, the push and the Pull Request of that phase. That Pull Request is always a draft, and it carries no body. `git-manager` never merges and never marks it ready, because a person reviews it. The command reports the draft Pull Request, and it stops. It runs one phase, and it never continues to the next one.
