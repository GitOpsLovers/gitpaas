...
The frontend is zoneless. Thus a spec must make the change detection and the asynchronous tasks happen on purpose.
...
| A resource whose value the test reads | `await settle()` |
...
Then a test drives the state of the screen with `value.set(...)` or `isLoading.set(true)`, and it renders again. A test asserts the count of the calls of `reload`.

## An RxJS stream

**Push the values through a real `Subject`:**
...
**Assert the end of the subscription with `observed` of the subject**, because a component that opens a long stream must close it:
...
A repository that reads `text/event-stream` uses `fetch` and a reader of the body. Replace the global with `vi.stubGlobal('fetch', fetchMock)`, and remove it in `afterEach` with `vi.unstubAllGlobals()`.
...
**Cover these cases:** the headers of the request; the split of a chunk across two reads; an event with a body that is not valid JSON; the end event; the failed answer (`ok: false`); and the cancel of the subscription.
