...
`TextareaFieldComponent` (`app-textarea-field`) is the shared primitive of a bounded text field. It takes the signal inputs `label`, `placeholder`, `hint`, `rows` (default `3`), `value` and the required `maxLength`, and it emits `valueChange` on every keystroke. It shows the label above the field, when the caller gives one, and a counter of characters below it, in the shape "`{value} of {maxLength} characters.`", prefixed by the `hint` when the caller gives one. The form of a project and the form of a namespace both use it for their field of the description, so the two share one shape and one counter.
