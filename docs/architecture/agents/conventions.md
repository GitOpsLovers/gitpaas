...
Every agent file names the skill of the job that it always loads, before it touches a file of the area that the skill covers. `implementer`, `refactorer` and `tester` name `backend-architecture` or `frontend-architecture` for a file of `apps/`; `documenter` names the same pair for a page that documents `apps/`, and `project-documentation` for every page it writes; `git-manager` names `git-github-workflow` alone. A skill of the reference — `angular-developer`, `nestjs-best-practices`, `turborepo`, `vitest`, `tailwind-4-docs`, `typescript-advanced-types` and `tailadmin-ui-patterns` — is optional, and the report of the agent names it and the reason in the field **Notes** when it equips one. `CLAUDE.md` gives the rule that the skill of the job wins when the two disagree.
