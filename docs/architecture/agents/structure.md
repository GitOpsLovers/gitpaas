...
- **The skill of the job**, one per area of the code, always loaded before the work starts: `backend-architecture`, `frontend-architecture`, `backend-feature`, `backend-unit-testing`, `frontend-unit-testing`, `project-documentation`, `git-github-workflow` and `agent-orchestration`. Each one routes to the page that is the single source of truth, so the skill itself carries a table of pointers and not the rule.
- **The skill of the reference**, on demand, one at a time: `angular-developer`, `nestjs-best-practices`, `turborepo`, `vitest`, `tailwind-4-docs`, `typescript-advanced-types` and, for the markup of the frontend, `tailadmin-ui-patterns`. A skill of the job wins over a skill of the reference when the two disagree.
