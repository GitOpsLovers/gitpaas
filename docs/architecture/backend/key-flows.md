...
A service holds a set of variables in `features/service-environment/`: a `ServiceVariable` (`domain/models/service-variable.models.ts`) is a name, a `secret` flag, and a value. `ServiceVariablesController` (`ui/controllers/service-variables.controller.ts`) exposes `GET/POST/PUT/DELETE` under `/api/v1/services/:serviceId/variables`, and `packages/contracts` fixes the shape of a name: capital letters, digits and the low line, and no leading digit.

- **Encryption at rest**: `setServiceVariableUseCase` and `updateServiceVariableUseCase` (`application/`) seal the value of a variable with `secret: true` through the same `SecretCipher` port that a provider key uses (see [Providers and encrypted credentials](#providers-and-encrypted-credentials)), before the row is written. A plain variable stores its value in clear text. The read model never carries the clear value of a secret: `value` stays `null`, and a `valueSet` flag alone tells the client that a value exists.
- **Injection at the deployment**: `runDeploymentUseCase` (`features/deployments/application/run-deployment.use-case.ts`) calls `getServiceEnvironmentUseCase` (`features/service-environment/application/get-service-environment.use-case.ts`) after it fetches the source archive of the run. That use case reads every stored variable, decrypts each secret, and returns one flat `name → clear value` record. When a secret cannot be opened it throws `ServiceVariableNotDecryptableError`, naming the variable and not its value; `runDeploymentUseCase` catches it like any other failure of the run, marks the deployment `failed`, and never calls the Docker executor, so no stack starts on an undecryptable secret.
- **Merge into the compose recipe**: `DockerExecutorAdapter.up` (`infrastructure/docker/docker-executor.adapter.ts`) passes that record to `injectEnvironment` (`compose-recipe.transformer.ts`), after it stops the previous containers and before it starts the new ones. `injectEnvironment` merges the record over the `environment` map that each service of the compose file already declares, so a variable of the service overrides a value the compose file gives the same name, and every container of the stack receives it as any other Docker Compose environment value.
- **The risk of the compose file**: after the stack starts, GitPaaS reads a bounded tail of the standard output and the standard error of every container, and appends it to the live log of the deployment (`captureStartupLogs`). A compose file that runs a command or an entrypoint that prints its own environment — an `env`, an `echo $VAR`, a verbose start script — puts the value of that variable into the log, plain or secret alike. GitPaaS keeps a secret out of its own logs and out of an answer of its API, but it does not filter the output that a container chooses to print.
