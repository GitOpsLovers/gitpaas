...
- **Deploy**: the operation extracts the source archive, builds or pulls the images the compose file needs, stops the old stack, labels each new resource, starts the stack, and captures a limited quantity of start-up output. It reports each line while it operates, so the run is visible in the live log. The extraction lands in one folder per run under the base folder that `DEPLOY_SPOOL_DIR` names — the temporary folder of the process when the variable is empty or absent — and the transformer of the recipe rewrites the source of every relative bind mount against the folder of the compose file, because the daemon reads that source against the root of the host and would otherwise create a volume of that name. See [the environment contract](../infrastructure/conventions.md#environment-contract).
- **The external networks of the recipe leave it before the start, the way the network of the proxy already does.** `dockerode-compose` crashes on a second `external` network of a service, so `stripExternalNetworks` (`compose-recipe.transformer.ts`) takes every `external` network out of the recipe before `DockerExecutor.up()` builds and starts the stack, and it returns the name each one holds on the daemon, grouped by the compose service that named it. Once the stack is up, `attachToRecipeNetworks` reads the label of each container and connects it to the networks of its compose service, under the alias of the name of that service; a network the daemon does not hold fails the deployment with a named error. `toFinalComposeText` declares those networks again in the text of the Compose file the user reads (`declareAttachedNetworks`), so the card of the service shows the truth. A network of the recipe that a container of another stack still holds after the removal of the old one would otherwise make the daemon answer the code `409` on its creation: `reuseSurvivingNetworks` finds it still on the daemon, takes it out of the recipe too, and hands it to `attachToRecipeNetworks`, so the deployment reuses the surviving network instead of creating it again.
- **Volumes**: once the stack is up, `reconcileComposeVolumesUseCase` (`features/volumes/application/reconcile-compose-volumes.use-case.ts`) brings the record of the volumes of the service to the named volumes the same Compose text already read for the domains declares (`parseComposeVolumesUseCase`), so the Compose file stays the one source of truth of a volume: it inserts a volume the Compose file adds, it updates the mount of a volume whose compose service, mount path or mode changed, and it deletes the record of a volume the Compose file no longer declares. GitPaaS creates no volume on the daemon; Docker Compose alone does, from the key its Compose file carries. A failure of this reconciliation never fails the deployment; it logs a line and leaves the record at its last successful state. See the capability [volumes](../../business/volumes.md).
...
- **Labelling**: GitPaaS uses the same daemon as the control plane and as the third-party stacks, so every resource it creates carries a marker label and a project label. The three operations of the prune of the images, of the volumes and of the containers select only labelled resources, so they never touch the control plane or a third-party stack. The prune of the host and the prune of the build cache carry no such filter: `DockerServerPrunerAdapter.pruneHost()` (`features/server/infrastructure/docker/docker-server-pruner.adapter.ts`) calls `pruneImages` and `pruneContainers` of `ContainerRuntime` with the `HOST_SELECTOR` (`features/docker/domain/constants/docker-host.constants.ts`), which carries no label, and `pruneBuildCache` of `ContainerRuntime` takes no selector at all. Both reach every image, every stopped container and every cache record of the daemon, whatever origin created them.
