...
- [ ] Add an `enrichWideEvent` call with the `error.*` fields in `core/ui/filters/all-exceptions.filter.ts`.
...
- [ ] Add the `user.*` and `auth.*` fields from the `request.user` that `features/authentication/infrastructure/passport/jwt.strategy.ts` attaches.
- [ ] Add the `project.*` fields in `features/projects/ui/controllers/projects.controller.ts` and `features/projects/ui/services/projects.service.ts`.
- [ ] Add the `service.*` and `deployment.*` fields in `features/deployments/ui/controllers/deployments.controller.ts` and `features/deployments/ui/services/deployments.service.ts`.
- [ ] Add the same enrichment to the remaining features, with the `projects` feature as the reference.
...
- [ ] Add a `runWithWideEvent` scope for each task in the `run` method of `features/deployments/ui/services/deployment-runner.service.ts`.
...
- [ ] Decide the store and the query tool: stdout only, a `wide_events` `JSONB` table in the existing PostgreSQL, a columnar store, or a hosted vendor.
...
- [ ] Decide if the commit message of a deployment can be a field, or only the SHA and the branch.
