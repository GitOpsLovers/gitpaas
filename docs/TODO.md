...
- [x] Divide the two large adapters: `docker-executor.adapter.ts` (533 lines, of which approximately 7 methods only do a transformation of the compose recipe and have no I/O) and `db-log-store.adapter.ts` (457 lines, with the state machine, the sequencer, the batcher, the replay merger, the trimmer and the retention sweeper in one class).
